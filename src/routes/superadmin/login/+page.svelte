...
<svelte:head>
  <title>Restricted Access - Superadmin</title>
</svelte:head>

<div class="min-h-screen flex items-center justify-center bg-[#0a0a0a] text-white font-sans p-4 relative overflow-hidden">
  
  <!-- Cyberpunk/Secure Background Elements -->
  <div class="absolute inset-0 pointer-events-none opacity-20">
    <div class="absolute top-0 left-1/4 w-96 h-96 bg-red-900 rounded-full blur-[120px]"></div>
    <div class="absolute bottom-0 right-1/4 w-96 h-96 bg-purple-900 rounded-full blur-[120px]"></div>
  </div>

  <div class="w-full max-w-md p-8 rounded-2xl bg-black/40 border border-red-500/20 backdrop-blur-xl relative z-10">
    <div class="flex flex-col items-center justify-center text-center space-y-4 mb-8">
      <div class="w-16 h-16 rounded-2xl bg-red-500/10 flex items-center justify-center border border-red-500/30 text-red-500">
        <ShieldAlert size={32} />
      </div>
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-white mb-1">Restricted Area</h1>
        <p class="text-sm text-gray-400 font-mono">AUTHORIZED PERSONNEL ONLY</p>
      </div>
    </div>

    {#if errorMessage}
      <div class="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-sm flex items-center gap-3">
        <Fingerprint size={18} class="shrink-0" />
        {#if errorMessage === 'unauthorized'}
          <span>Access Denied. Your account is not authorized to enter this sector.</span>
        {:else}
